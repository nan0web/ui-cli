...
### Додано
- **Загартування OLMUI v2.0**: Оновлено `@nan0web/types` до `1.7.1` для підтримки розширених метаданих Model-as-Schema та атомарної ізоляції логіки.
- **Інтеграція Snapshot**: Верифіковано сумісність з утилітою `verifySnapshot` (JSONL) для захоплення послідовностей логічних інтенцій у всіх CLI компонентах.
- **Модернізація Моделей**: Оновлено основні моделі запитів (`Input`, `Select`, `Confirm`, `Tree`, `Mask`) для використання нового шаблону `{ default, alias }` для властивості `help`, що забезпечує архітектурну цілісність OLMUI.
