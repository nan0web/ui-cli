...
// @todo add --cwd parameter.
db.mount('', new DBFS({ root: isTestMode ? process.cwd() : 'data', console: dbConsole })) // Application Config & Translations
if (isTestMode) {
	db.mount('snapshots', new DBFS({ root: 'snapshots', console: dbConsole }))
}
/**
...
// @todo convert to model as app
;(async () => {
	try {
		// 1. OLMUI Thin Client (Remote Execution mode)
		const targetUrl = process.argv[2]
		if (targetUrl && /^https?:\/\//.test(targetUrl)) {
			// @todo use yield show or progress instead of console.*
		...
		const tmpDir = (await import('node:fs')).mkdtempSync(
			(await import('node:path')).join(os.tmpdir(), 'grow-private-')
		)
		;(await import('node:fs')).writeFileSync(
			(await import('node:path')).join(tmpDir, 'farm_stats.json'),
			JSON.stringify({ income: 42000 })
		)
		...
		const appExport = pkg?.exports?.['./cli'] || pkg?.exports?.['./app'] || pkg?.exports?.['./ui/cli']
		let appPath = null
		...
		if (process.argv[2] === 'blueprint') {
			...
			process.argv[2] &&
			(process.argv[2].startsWith('.') || process.argv[2].startsWith('@nan0web'))
		) {
			// Handle dynamic app loading from cli (e.g. nan0cli @nan0web/ui/inspect or nan0cli ./src/inspect.js)
			try {
				let resolvedPath = process.argv[2]
				if (resolvedPath.startsWith('@nan0web')) {
					// Extremely simple node resolution via createRequire
					const { createRequire } = await import('node:module')
					const req = createRequire(path.join(process.cwd(), 'package.json'))
					resolvedPath = req.resolve(process.argv[2])
				} else {
					resolvedPath = path.resolve(process.cwd(), process.argv[2])
				}
				appPath = resolvedPath
			} catch (e) {
				showError('Module Resolution Error', `Could not resolve ${process.argv[2]}`)
				process.exit(1)
			}
		} else if (appExport) {
			const exportPath =
				typeof appExport === 'string' ? appExport : appExport.import || appExport.default
			if (exportPath) {
		...
		if (appPath) {
			...
			const AppModel =
				appModule.default ||
				appModule.App ||
				Object.values(appModule).find((m) => typeof m === 'function' && m.prototype?.run)

			if (!AppModel) {
				showError(
					'Config Error',
					`Module at ${appPath} (from exports['./ui/cli'] or blueprint) must export default, App class, or a class with run()`
				)
				process.exit(1)
			}
			...
			const appArgv = process.argv.slice(3)
			if (AppModel.prototype && typeof AppModel.prototype.run === 'function') {
				let app = modelFromArgv(AppModel, appArgv, options)
				while (true) {
					...
					if (res.data?.type === 'result') {
						...
						// Test Mode: Exit 1 on errors
						const isTestMode = process.argv.includes('--test')
						const isError = res.data.data?.status === 'error' || res.data.data?.success === false
		...
		if (!appPath) {
			showError(
				'Config Error',
				'No CLI entry point found.\nPlease add "exports": { "./ui/cli": "./src/domain/App.js" } to package.json\nOR add "nan0web.cli.entry" to package.json'
			)
			process.exit(1)
		}
