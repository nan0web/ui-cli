    ...
    function render() {
        ...
        for (let i = 0; i < visible.length; i++) {
            ...
            if (multiselect) {
                ...
                line += isChecked ? Logger.style('◉ ', { color: Logger.GREEN }) : Logger.style('◯ ', { color: Logger.DIM })
            }
    ...
    return new Promise((resolve, reject) => {
        const onKey = async (str, key) => {
            ...
            switch (key.name) {
                ...
                case 'left': left(); break // Sync handled?
                case 'right': await right(); break
                case 'space': check(); break
        ...
        function cleanup() {
            ...
            if (state.done) {
                ...
                // Show result summary
                if (multiselect) {
                    stdout.write(Logger.style(`${state.checked.size} ${t('tree.selected')}\n`, { color: Logger.WHITE }))
                } else {
                    const node = state.flat[state.cursor]
