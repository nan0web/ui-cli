		...
		return {
			fill: async () => {
				...
				while (true) {
					...
					const constructorName = String(data?.constructor?.name || '')
					const isComponent =
						typeof constructorName === 'string' && 
						constructorName.endsWith('Model') && 
						constructorName !== 'SandboxModel'
					if (isComponent) {
						const componentName = constructorName.replace('Model', '')
					...
					const choices = [
						...fields.map((f) => {
							...
							if (typeof val === 'boolean') {
								displayVal = val ? adapter.t(ToggleModel.UI_YES) : adapter.t(ToggleModel.UI_NO)
						...
						{ label: `✅ ${adapter.t(FormModel.UI_DONE)}`, value: '_save' },
						{ label: `❌ ${adapter.t(ToggleModel.UI_NO)}`, value: '_cancel' },
					]

					const selectedField = await adapter.requestSelect({
						title: `📝 ${adapter.t(FormModel.UI_EDIT)} ${adapter.t(SchemaClass.name)}`,
						message: adapter.t(SelectModel.UI_PROMPT),
						options: choices,
						limit: 15,
						initial: lastIndex,
					})
					...
					if (field.options && field.options.length) {
						...
						result = await adapter.requestToggle({
							...
							active: field.active || adapter.t(ToggleModel.UI_YES),
							inactive: field.inactive || adapter.t(ToggleModel.UI_NO),
						})
