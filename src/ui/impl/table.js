...
export async function table(input) {
	...
	if (instant && process.stdin.isTTY) {
		return new Promise((resolve) => {
			const render = () => {
				...
				logger.clear()
				const filterText = input.filterMsg || 'filter'
				const infoMsg = title
					? `${title} (${filterText}: "${query}")`
					: `(${filterText}: "${query}")`
				logger.info(infoMsg)
	...
	while (true) {
		...
		if (title) {
			const filterText = input.filterMsg || 'filter'
			const noneText = input.noneMsg || 'none'
			logger.info(`${title} (${filterText}: "${query || noneText}")`)
		}
		logger.table(displayData, displayColumns)

		const promptFn = input.prompt || text
		const res = await promptFn({
			message: input.filterPrompt || 'table.filter_prompt',
			initial: query,
		})
