...
export async function tree(config) {
	const {
		...
		empty = TreeModel.UI_EMPTY,
		loading = TreeModel.UI_LOADING,
		selected = TreeModel.UI_SELECTED,
		t = (k) => k,
	} = config
		...
		for (let i = 0; i < visible.length; i++) {
			...
			if (isFocused) {
				...
				if (state.loading && state.expanded.has(node))
					line += Logger.style(` ${t(loading)}`, { color: Logger.DIM })
			} else {
				line += '  ' + nameDisplay
			}
	...
	return new Promise((resolve, reject) => {
		...
		function cleanup() {
			...
			if (state.done) {
				...
				if (multiselect) {
					stdout.write(
						Logger.style(`${state.checked.size} ${t(selected)}\n`, { color: Logger.WHITE })
					)
				} else {
					const node = state.flat[state.cursor]
